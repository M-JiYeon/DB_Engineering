...
/*
    EX 2]
        ...
        이름, 직급, 입사일을 조회하세요.
*/
SELECT ename 사원이름, job 직급, TO_CHAR(hiredate, 'YYYY-MM-DD') 입사일
FROM emp
...
SELECT ename 사원이름, job 직급, hiredate 입사일
FROM emp
WHERE ename BETWEEN 'C' AND 'Mz' -- 아스키 코드는 소문자가 대문자보다 큼
;
...
SELECT ename 사원이름, job 직급
FROM emp
WHERE ename BETWEEN 'S' AND 'T' AND LENGTH(ename) = 5 ;
...
SELECT 
    ...
    INSTR('abcde@naver.com', '@') - 1 AS 길이,  -- @ 앞 길이
    RPAD('*', (INSTR('abcde@naver.com', '.') - 1) - INSTR('abcde@naver.com', '@'), '*') AS 도메인이름,
    SUBSTR('abcde@naver.com', INSTR('abcde@naver.com', '.'))AS  확장자
FROM dual ;
...
SELECT 
    ...
    SUBSTR('abcde@naver.com', INSTR('abcde@naver.com', '.')) AS  메일
FROM dual ;
    ...
    CONCAT(
        RPAD(
            CONCAT( 
                ...
                LPAD( SUBSTR('abcde@naver.com', INSTR('abcde@naver.com', '@') + 2, 1), '*' )
            ),
            INSTR('abcde@naver.com', '.') -1, 
            '*'
        ),
