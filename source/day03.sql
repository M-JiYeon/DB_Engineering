...
-- 제시 정답
SELECT 
    ename 사원이름, dname 부서이름, sum 부서급여합계, max 부서최대급여, min 부서최소급여, ROUND(avg,2) 부서평균급여 
FROM 
    emp e, dept d,
    (
        SELECT 
            deptno dno, SUM(sal) sum, MAX(sal) max, MIN(sal) min, AVG(sal) avg
        FROM 
            emp
        GROUP BY 
            deptno
